...
from astrop.wcs.utils import skycoord_to_pixel
...
class MaskSources(object):
    ...
    def __init__(self, data, header, name, path_to_table, ps=None, include_galaxy=False,
                 large_incl_corr=True, incl_limit=0.2):
        if data.ndim != 2:
    ...
    def calc_mask(self):
        ...
        pos = n_table['RA'] * u.deg, n_table['DEC'] * u.deg, n_table['pa'] * u.deg, n_table['inclination'] * u.deg
        n_ra, n_dec, n_pa, n_incl = Angle(pos[0]), Angle(pos[1]), Angle(pos[2]), Angle(pos[3])
